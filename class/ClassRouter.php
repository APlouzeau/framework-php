...
/**
 * ClassRouter - Système de routage avec support des paramètres dynamiques
 *
...
class ClassRouter
{
    ...
    public function addRoute(string $method, string $path, string $controller, string $action, array $middlewares = []): void
    {
        ...
        foreach ($middlewares as $middleware) {
            ClassMiddlewareManager::addToRoute($path, $middleware);
        }
    }
    ...
    /**
     * Ajouter une route protégée (avec authentification)
     *
     * @param string $method HTTP method
     * @param string $path Route path
     ...
     */
    public function addProtectedRoute(string $method, string $path, string $controller, string $action): void
    {
        $this->addRoute($method, $path, $controller, $action, ['ClassMiddlewareAuth']);
    }
